...
class Separator:
    ...
    def _load_numpy(self, numpy,sr):
        print("_load_numpy")
        errors = {}
        # 将 NumPy 数组转换为 PyTorch Tensor
        wav = th.from_numpy(numpy)
        print(f"Original shape of wav: {wav.shape}")  # 打印原始 wav 的形状
        # print(f"Number of channels: {self._audio_channels()}")  # 打印通道数
        # wav = wav.view(-1, self.self._audio_streams[stream]['channels']()).t()
        # wav = wav.view(-1, self._audio_channels()).t() #张量被重新整形，确保其形状是 [C, T]。
        # wav = convert_audio(wav, sr, self._samplerate, self._audio_channels)
        # self._samplerate = sr
        wav = convert_audio(wav, sr, self._samplerate, self._audio_channels)
      
        print(f"------------------------>_input_Shape: {wav.shape}")  # 打印数组的形状
        if wav is None:
    ...
    def _load_audio(self, track: Path):
        ...
        try:
            ...
            print(f"111111111wav {wav.shape}")
        ...
        if wav is None:
            try:
                ...
                print(f"222222wav {wav.shape}")

            except RuntimeError as err:
                errors["torchaudio"] = err.args[0]
            else:
                print("4444444convert_audio")
                # self._samplerate = sr
                wav = convert_audio(wav, sr, self._samplerate, self._audio_channels)
    ...
    def separate_tensor(
        ...
        wav *= ref.std()
        wav += ref.mean()
        print(f"separate_tensor finish,{self._model.sources}")
        # print("----out[0]------>",out[0].shape)

        # out_single_channel = out[0][:, :1,:]
        # print("-----out_single_channel----->",out_single_channel.shape)
        
        return (wav, dict(zip(self._model.sources, out[0])))
        # return (wav, dict(zip(self._model.sources, out_single_channel)))
    ...
    def separate_audio_numpy(self, numpy,sr=44100):
        """
        Separate an audio file. The method will automatically read the file.
        ...
        been resampled.
        """
        print("enter separate_audio_numpy")
        return self.separate_tensor(self._load_numpy(numpy,sr), sr)
