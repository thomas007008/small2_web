...
import sys
sys.path.insert(1, '/home//workspace/music_site/music_app/algo/new_convert_midi/utils/')

from utils.piano_vad import (note_detection_with_onset_offset_regress, 
...
def read_metadata(csv_path):
    ...
    return meta_dict
def in_it(value, list):
    # list_index = []
    if len(list) == 1:
        return 0
    for index, val in enumerate(list):
        if index > 0:
            if (value < val) and (value >= list[index-1]):
                return index
            if index == len(list) - 1:
                return len(list)

def read_midi_new(midi_path, tempo_list):
    if os.path.exists(midi_path):

        midi_file = MidiFile(midi_path)
        base_microseconds_per_beat = 500000
        base_beats_per_second = 2
        ...
        message_datas = midi_file.tracks
        ticks_per_beat = midi_file.ticks_per_beat
        ...
        for message in midi_file.tracks[0]:
            ...
            if message.type == 'set_tempo':
                ...
                if len(tempo_list) == 1:
                    ...
                    # time = (ticks/ticks_per_beat) * (tempo_list[len(tempo_list)-2]/1e6)
                    beats_per_second = 1e6 / tempo_list[len(tempo_list)-2]
                    ticks_per_second = ticks_per_beat * beats_per_second
                    time = (ticks - ticks_list[-1]) / ticks_per_second + limit_time[-1]

                microseconds_per_beat.append([message.tempo, ticks, time])
                # speed_event.append()
                limit_time.append(time)
                message_tempo.append(message.tempo)
        ...
        for message in midi_file.tracks[1]:
            ...
            ticks_note += message.time
            ticks_notes.append(ticks_note)


            base_ticks_per_second = ticks_per_beat * base_beats_per_second
            # time_in_second.append(ticks_note / ticks_per_second)
            base_time = ticks_note / base_ticks_per_second

            if len(ticks_list) == 1:
                beats_per_second = 1e6 / message_tempo[0]
                ticks_per_second = ticks_per_beat * beats_per_second            
                last_time = ticks_note / ticks_per_second
                time_offset.append(base_time - last_time)

                midi_base_time.append(base_time)
                time_in_second.append(last_time)
                ...
                # if ticks_note > 1920:
                #     print()
                index = in_it(ticks_note, ticks_list)

                beats_per_second = 1e6 / message_tempo[index-1]
                ticks_per_second = ticks_per_beat * beats_per_second
                last_time = limit_time[index-1]  + (ticks_note - ticks_list[index-1])/ ticks_per_second
                time_offset.append(base_time - last_time)
                midi_base_time.append(base_time)

                time_in_second.append(last_time) 
            # print()
    # print(time_in_second[-1])           
    # wavname = midi_path.replace("MIDI.mid", "WAV.wav")
    # duration = read_wav(wavname)
    # print(duration)
    # if abs(duration - time_in_second[-1]) > 3:
    #     different.append(wavname)
    #     different.append(midi_path)
    #     different.append(time_in_second[-1])
    #     different.append(duration)
    #     different.append(abs(duration -  time_in_second[-1]))
    #     diff.append(different)    
        # midi_dict = {
        #     'midi_event': np.array(message_list),
        #     'midi_time_offset': np.array(time_offset),
        #     'midi_event_time': np.array(time_in_second), 
        #     'midi_base_time' : np.array(midi_base_time),
        #     'midi_ticks_notes':np.array(ticks_notes),
        #     'midi_speed':np.array(microseconds_per_beat)}
        midi_dict = {
            'midi_event': np.array(message_list),
            # 'midi_time_offset': np.array(time_offset),
            'midi_event_time': np.array(time_in_second), 
...
def read_midi(midi_path):
    ...
    # microseconds_per_beat = midi_file.tracks[0][3].tempo
...
def write_events_to_midi(start_time, note_events, pedal_events, midi_path,type=0):
    ...
    for message in message_roll:
        print(f"message['time']: {message['time']}, start_time: {start_time}, ticks_per_second: {ticks_per_second}")
    ...
    if(1 == type):
        return midi_file
    print("00000000000000000000midi_path",midi_path)
