...
class ConvBlock(nn.Module):
    ...
    # def forward(self, input, pool_size, pool_type='avg'):
...
class AcousticModelCRnn8Dropout(nn.Module):
    ...
    #     x = F.dropout(x, p=0.5, training=self.training, inplace=False) #TRUE
    ...
    #     output = torch.sigmoid(self.fc(x))
        ...
        # x = F.dropout(x, p=0.2, training=self.training)
        # ddd = input.transpose(1, 3)
        # ccc = self.conv1(ddd).flatten(0,1)
        x = input.transpose(1, 2).flatten(2)
        x = F.relu(self.bn5(self.fc5(x).transpose(1, 2)).transpose(1, 2))
        x = F.dropout(x, p=0.5, training=self.training, inplace=False) #TRUE
...
#                                                  amin=amin, top_db=top_db, freeze_parameters=True)
        ...
        self.conv_block1 = ConvBlock(
            in_channels=1, out_channels=128, momentum=momentum)
        self.conv_block2 = ConvBlock(
            in_channels=128, out_channels=128, momentum=momentum)
        self.conv_block3 = ConvBlock(
            in_channels=128, out_channels=256, momentum=momentum)
        self.conv_block4 = ConvBlock(
            in_channels=256, out_channels=256, momentum=momentum)
        self.conv_block5 = ConvBlock(
            in_channels=256, out_channels=512, momentum=momentum)            
        self.conv_block6 = ConvBlock(
            in_channels=512, out_channels=512, momentum=momentum)
        self.conv_block7 = ConvBlock(
            in_channels=512, out_channels=1024, momentum=momentum)
        # self.conv_block8 = ConvBlock(
        #     in_channels=128, out_channels=128, momentum=momentum)
        # self.conv_block9 = ConvBlock(
        #     in_channels=128, out_channels=256, momentum=momentum)
        # self.conv_block10 = ConvBlock(
        #     in_channels=256, out_channels=256, momentum=momentum)
        # self.conv_block11 = ConvBlock(
        #     in_channels=256, out_channels=512, momentum=momentum)
        ...
        # self.wheel_model = AcousticModelCRnn8Dropout(
        #     8192*2, 1792, momentum)

        # self.reg_onset_gru = nn.GRU(input_size=classes_num * 2 + 8192*2, hidden_size=256, num_layers=1, 
        #     bias=True, batch_first=True, dropout=0., bidirectional=True)
        # self.reg_onset_fc = nn.Linear(512, classes_num, bias=True)

        # self.frame_gru = nn.GRU(input_size=classes_num * 3 + 8192*2, hidden_size=256, num_layers=1, 
        #     bias=True, batch_first=True, dropout=0., bidirectional=True)
        # # self.init_weight()
    ...
    def init_weight(self):
        ...
        # init_gru(self.wheel_onset_gru)
        # init_gru(self.frame_gru)
        # init_layer(self.reg_onset_fc)
        # init_layer(self.reg_offset_fc)
        # init_layer(self.wheel_onset_gru_fc)
...
#         x = self.conv_block7(x, pool_size=(1, 2), pool_type='avg')
        ...
        # x = self.conv_block8(x, pool_size=(1, 2), pool_type='avg')
        # x = self.conv_block9(x, pool_size=(1, 2), pool_type='avg')
        # x = self.conv_block10(x, pool_size=(1, 2), pool_type='avg')
        # x = self.conv_block11(x, pool_size=(1, 2), pool_type='avg')
        ...
        # wheel_model_output = self.wheel_model(x)
        velocity_model_output = self.velocity_model(x)
        
        # Use velocities to condition onset regression
        # x = torch.cat((reg_onset_output, (reg_onset_output ** 0.5) * velocity_model_output.detach(),wheel_model_output.detach()), dim=2)
        ...
        output_dict = {
            ...
            'velocity_output':velocity_model_output
            }
            # 'velocity_output': velocity_output}

        return output_dict
class Regress_onset_offset_frame_velocity_CRNN_20231227(nn.Module): #
    def __init__(self, frames_per_second, classes_num):
        super(Regress_onset_offset_frame_velocity_CRNN_20231227, self).__init__()
    ...
    def forward(self, input):
        ...
        return output_dict
class AcousticModelCRnn8Dropout_old(nn.Module):
    def __init__(self, classes_num, midfeat, momentum):
        super(AcousticModelCRnn8Dropout_old, self).__init__()
...
class Regress_onset_offset_frame_velocity_CRNN_old(nn.Module):
    def __init__(self, frames_per_second, classes_num):
        super(Regress_onset_offset_frame_velocity_CRNN_old, self).__init__()
...
class Regress_pedal_CRNN(nn.Module):
    def __init__(self, frames_per_second, classes_num):
        ...
        self.reg_pedal_onset_model = AcousticModelCRnn8Dropout_old(1, midfeat, momentum)
        self.reg_pedal_offset_model = AcousticModelCRnn8Dropout_old(1, midfeat, momentum)
        self.reg_pedal_frame_model = AcousticModelCRnn8Dropout_old(1, midfeat, momentum)
...
class Note_pedal(nn.Module):
    ...
    def load_state_dict(self, m, strict=False):
        ...
        self.note_model.load_state_dict(m, strict=strict)
