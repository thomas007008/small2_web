...
"""
        ...
        self.model.eval()
        self.model.to("cuda")
        #torch.Size([1, 1, 1001, 229])
        # Load model
        checkpoint = torch.load(checkpoint_path, map_location=self.device)
        self.model.load_state_dict(checkpoint['model'], strict=False)
        # self.model.eval()

        trt_ts_module = torch_tensorrt.compile(self.model, 
            inputs= [torch_tensorrt.Input((1, 1, 1001, 229))],
            enabled_precisions= {torch_tensorrt.dtype.float} # Run with FP16
        )
        ...
        # )
        input_data = torch.zeros(1,1,1001,229)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--checkpoint_path', type=str,  default="/root/yyz/code/piano_transcription/checkpoints/main/Regress_onset_offset_frame_velocity_CRNN/loss_type=regress_onset_offset_frame_velocity_bce/augmentation=none/max_note_shift=0/batch_size=16/150000_iterations.pth")
